import { getErrorMessage, isBadNum } from './utils.js';
...
export class Grapher {
  ...
  events: EventEmitter<GrapherOnEventFunctions>;

  constructor() {
    this.events = new EventEmitter<GrapherOnEventFunctions>();
  }
  ...
  public compileFormula(formula: Formula, storeFunc = true): boolean {
    ...
    try {
      // Create a safe evaluation context that includes our math functions
      const context = { Math: { ...Math, ...mathFunctions } };
      const contextKeys = Object.keys(context);
      const contextValues = Object.values(context);
      
      // Use Function constructor with controlled context
      fnFormula = new Function(
        ...contextKeys,
        'x',
        't',
        `return (function() { ${fncString} })()`
      ).bind(null, ...contextValues) as FncFormula;
    } catch (err) {
  ...
  private iMouseUp(_e: MouseEvent) {
    this.mMouseFunction = 0;
  }
